...
# Patterns that catch the long tail of "dispensing error", "administration
# error", etc. Case-insensitive on the PT name.
...
# Stop-words removed before word-level matching of a PT against label text.
.EVENT_STOP_WORDS <- c(
  "of", "the", "and", "in", "to", "a", "an", "on", "at", "with", "by",
  "for", "as", "is", "be", "from", "or", "under"
)

# Returns TRUE if the event text appears in the label (exact substring OR
# \u226570% of its non-trivial words match). Case-insensitive. Used by both
# the top-novel filter and the KNOWN/NOVEL badge.
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # classic vioxx / Myocardial infarction pair when available.
    shiny::observe({
      pairs <- pairs_cache()
      req(pairs)
      drugs <- sort(unique(pairs$rxnorm_name))
      events <- sort(unique(pairs$outcome_name))
      default_drug <- if ("vioxx" %in% drugs) "vioxx" else drugs[1]
      default_event <- if ("Myocardial infarction" %in% events) "Myocardial infarction" else events[1]
